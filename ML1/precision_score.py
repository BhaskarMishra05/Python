...
cvkf=cross_val_score(model,x,y,cv=kf)
print(f'CV no Kfold {cv}')
print(f'CV with Kfold: {cvkf}')

#Importing R2 and MSE
from sklearn.metrics import r2_score,mean_squared_error,make_scorer, precision_score
precision = make_scorer(precision_score, average='macro')
cv=cross_val_score(model,x,y,cv=5,scoring=precision)
cvkf=cross_val_score(model,x,y,cv=kf)
print(f'CV no Kfold {cv}')
print(f'CV with Kfold: {cvkf}')
